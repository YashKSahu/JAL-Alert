...
function epochToJsDate(epochTime){
  ...
  const hardnessElement = document.getElementById("hardness");
  const salanityElement = document.getElementById("salanity");
  const chlorinityElement = document.getElementById("chlorinity");
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        chartPh.destroy();
        chartTds.destroy();
        chartHardness.destroy();
        chartSalanity.destroy();
        chartChlorinity.destroy();
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          ...
          var hardness = jsonData.hardness;
          var salanity = jsonData.salanity;
          var chlorinity = jsonData.chlorinity;
          var timestamp = jsonData.timestamp;
          // Plot the values on the charts
          plotValues(chartT, timestamp, temperature);
          plotValues(chartPh, timestamp, ph);
          plotValues(chartTds, timestamp, tds);
          plotValues(chartHardness, timestamp, hardness);
          plotValues(chartSalanity, timestamp, salanity);
          plotValues(chartChlorinity, timestamp, chlorinity);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        hardnessElement.innerHTML = hardness;
        salanityElement.innerHTML = salanity;
        chlorinityElement.innerHTML = chlorinity;
      ...
      function createTable(){
        ...
        dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
          if (snapshot.exists()) {
            ...
            content += '<td>' + ph + '</td>';
            content += '<td>' + tds + '</td>';
            content += '<td>' + hardness + '</td>';
            content += '<td>' + salanity + '</td>';
            content += '<td>' + chlorinity + '</td>';
            content += '</tr>';
